...
class BusRoutes {
    ...
    public CompletableFuture<String> description() {
        CompletableFuture<StringBuilder> result = CompletableFuture.supplyAsync(
            () -> new StringBuilder(String.format("Search for: %s <-> %s:\nFrom %s\n", 
                                                      this.stop, this.name, this.stop)));

        for (BusService service: services.keySet()) {
            result = result.thenCombine(describeService(service, services.get(service)), 
                (x, y) -> x.append(y));
        }
        return result.thenApply(x -> x.toString());
    }
